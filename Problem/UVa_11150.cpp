...
int main() {
	int N;
	while (cin >> N) {
		int total_drunk = N, empty_bottle = N;
		while (empty_bottle >= 3) {
			int new_cola = empty_bottle / 3;
			total_drunk += new_cola;
			empty_bottle = (empty_bottle % 3) + new_cola;
		}
		if (empty_bottle == 2) {
			total_drunk++;
		}
...
The gure below shows the case where N = 8. Method 1 is the standard way: after nishing your 8
...
You can actually do better! In Method 2, you rst borrow an empty bottle from your friend (?! Or
