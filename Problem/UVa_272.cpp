...
int main() {
	int is_to_become_double_backquote = 1; //判斷是否要變成 " `` "
	char the_quote;
	while ((the_quote = getchar()) != EOF) {
		if (the_quote == '"') {
			if (is_to_become_double_backquote == 1) {
				printf("``");
				is_to_become_double_backquote = 0;
			}
			else {
				printf("''");
				is_to_become_double_backquote = 1;
			}
		}
		else {
			putchar(the_quote);
...
* 輸入說明：輸入是若干列的文字，其中有偶數個雙引號（ " ），以 end-of-file 做結束。輸出的文字必須和輸入的一模一樣，除了：每一組雙引號的第一個 " 必須用兩個 ` 字元（就是 `` ）來代替每一組雙引號的第二個 " 必須用兩個 ' 字元（ 就是 ''）來代替。
* 輸出說明：同上
